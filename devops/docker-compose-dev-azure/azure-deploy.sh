...
resourcename="space"  # lower case, only
resourcegroup="space_rg"
...
# git clone https://github.com/apilogicserver/space.git
# cd space
...
az group create --name space_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group space_rg --sku S1 --is-linux
az container create --resource-group space_rg --name space --image apilogicserver/space:latest --dns-name-label space --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//space.westus.azurecontainer.io:5656'
